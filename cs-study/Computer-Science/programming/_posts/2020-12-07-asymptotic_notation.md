...
- 가장 많이 쓰이는 점근적 표기법으로는 **점근적 상한**이라고도 불리는 Big-O Notation이 있다.
...
보다 직관적으로 풀어쓰자면 $$O(g(n))$$은 모든 $$n \geq n_0$$에 대해 $$0 \leq f(n) \leq c g(n)$$을 만족하는 양의 상수 $$n_0, c$$가 존재하는 함수 $$f(n)$$의 집합이라고 할 수 있다. 그래프로 확인하면 다음과 같다.
