...
학습 데이터를 구성하는 방법은 간단하다. 전체 Corpus 에서 임의의 문장을 뽑은 뒤, 50%의 확률로 직후 문장을 함께 넣어주고, 50%의 확률로 직후 문장을 제외한 다른 임의의 문장을 함께 넣어주는 것이다. 그리고 `IsNext`, `NotNext` 를 각각 label 로 모델을 학습하면 된다. 이때 두 문장 사이에는 `[SEP]` 이라는 Special Token 이 들어가 모델이 서로 다른 두 문장이라는 것을 알게 해준다.
