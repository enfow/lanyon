...
여기서 $$\theta$$는 gradient와 단위벡터 $$v$$ 간의 기울기를 의미한다. $$v$$는 단위벡터이므로 그 L2 NORM(L2 NORM은 유클리드 공간에서의 벡터의 크기를 의미)이 1이며, $$\nabla_x f(x)$$는 찾고자 하는 단위 벡터 $$v$$와 무관하다. 따라서 위의 최소화 문제는 $$ cos \theta$$를 최소화하는 문제가 된다. $$cos \theta$$는 $$1\pi$$에서 극소점을 가지므로, $$f$$를 최소화하기 위해서는 $$\nabla_x f(x)$$의 방향과 반대되는 방향으로 업데이트해야 한다.
