...
<img src="{{site.image_url}}/study/preemptive_and_non_preemptive.png" style="width:40em; display: block; margin: 0px auto;">
...
<img src="{{site.image_url}}/study/cpu_burst_io_burst.png" style="width:40em; display: block; margin: 0px auto;">
...
<img src="{{site.image_url}}/study/cpu_burst_histogram.png" style="width:40em; display: block; margin: 0px auto;">
...
가장 단순한 알고리즘인데 위의 그림에서도 확인할 수 있듯 Ready Queue 상의 프로세스 순서에 따라 Waiting time이 크게 바뀔 수 있다. 다시 말하면 최적화의 가능성이 남아 있다는 것이다.

### 3.2. SJF Scheduling

Shortest Jop First Scheduling은 CPU burst가 짧은 것을 먼저 처리하도록 하여 FCFS scheduling의 성능을 높이는 알고리즘이다. 평균 waiting time이 가장 짧은 알고리즘이다.
...
말 그대로 프로세스마다 우선순위가 있어 우선순위가 높은 프로세스를 먼저 처리하는 방법이다. 우선순위는 대개 0부터 8, 0부터 4095와 같이 일정한 범위 내의 정수로 정해지는데, 0이 가장 높은 것인지 낮은 것인지는 구현에 따라 다르다고 한다. 참고로 우선순위가 동일하면 FCFS 방식에 따라 먼저 들어온 프로세스를 우선 처리하게 된다.
...
Round Robin은 Time quantum을 얼마나 크게 설정하느냐에 따라 성능이 달라진다. Time quantum이 너무 짧으면 Context Switching으로 인한 오버헤드가 크게 늘어난다. 반대로 너무 길게 잡으면 FCFS와 동일해져 waiting time이 줄어드는 이점이 작아진다. 일반적으로는 전체 CPU burst 길이의 80% 수준으로 Time quantum을 설정한다고 한다.
...
<img src="{{site.image_url}}/study/multi_level_scheduling.png" style="width:40em; display: block; margin: 0px auto;">
...
<img src="{{site.image_url}}/study/multi_level_feedback_scheduling.png" style="width:40em; display: block; margin: 0px auto;">
