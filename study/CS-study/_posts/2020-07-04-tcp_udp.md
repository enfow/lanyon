---
layout: post
title: (IP) Transport Layer Protocols, TCP and UPD
category_num: 512
---

# Transport Layer Protocols, TCP and UPD
...
Transport Layer에서 가장 많이 사용되는 프로토콜인 TCP는 IP를 사용하는 패킷 중 이상 90%을 차지하며 가장 보편적인 전송 수단으로 꼽힌다. HTTP를 비롯하여 웹 서비스의 기본적인 Application들이 TCP를 사용하여 데이터를 주고 받으며, IP 위에서 동작하는 가장 대표적인 프로토콜이기 때문에 **TCP/IP**로 묶에 부르기도 한다.

UDP는 Transport Layer에서 TCP에 이어 두 번째로 많이 사용되는 프로토콜이다. 두 번째라는 말이 무색하게 전체 패킷의 대부분은 TCP를 사용하고 약 5%만이 UDP를 사용한다고 알려져 있지만 UDP는 TCP와 비교해 고유의 장점을 가지고 있고 UDP로만 처리되어야 하는 Application이 있다는 점에서 중요하다.
...
수신자가 가지고 있는 **receiver socket buffer에 남아있는 공간의 크기**를 의미한다. receiver socket buffer란 수신하였지만 Application layer에서 가져가지 않아 아직 Transport layer에 남아 있는 data가 임시적으로 저장되는 공간이다.
...
window size field의 크기는 16비트이므로 표현하고자 하는 window size가 $$2^{16}-1$$보다 큰 경우에는 부족하다. Window Scale Option field는 그 크기만큼 window size field를 left shift하여 window size를 인식하도록 해 보다 큰 window size를 표현하기 위해 사용한다.
...
connection을 종료할 때에는 바로 종료하지 않고 위의 그림과 같이 2분 정도 connection을 유지하게 된다. 이와같이 하는 이유는 두 가지인데 우선 첫 번째는 마지막 ACK를 수신자가 받지 못하는 경우 다시 보내기 위해서이다. 두 번째 이유는 TCP connection의 naming과 관련있다. TCP connection은 `<Receiver IP, Receiver Port, Sencder IP, Sender Port>`로 이름이 붙여지게 된다. 이때 동일한 이름을 가진 connection이 종료 후 곧바로 다시 생성되면 새롭게 만들어진 connection을 기존 connection으로 인식하게 되는 문제가 생길 수 있다. 이 경우 Synchronization이 제대로 수행되지 못할 수 있는데, 이러한 문제를 피하기 위해 기존 connection을 잠시 동안 살려두는 것이다.
...
기대에 맞는 Sequnce Number를 가진 패킷이 들어온 상황이다.
