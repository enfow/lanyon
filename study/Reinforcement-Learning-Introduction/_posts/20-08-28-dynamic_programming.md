...
강화학습에서는 주어진 환경에서 Optimal한 Policy를 찾는 것을 해결하고자 하는 큰 문제가 된다. 그리고 강화학습에 사용되는 DP는 이를 각각의 State에 대한 Value Function를 구하는 하위 문제로 쪼개어 해결하려 한다.

---

## 1. Policy Iteration

보다 구체적으로 Bellman Equation에 따라 각 State의 Value를 업데이트하며 아래와 같은 Bellman Optimality Equation을 구하고 이를 통해 Optimal Policy를 찾으려는 시도라고 할 수 있다.
...
**Policy Iteration**은  P**olicy Evaluation**과 **Policy Improvement**라는 두 가지 연산을 반복적으로 수행하여 이러한 Bellman Optimality Equation을 구하는 방법이다.

$$
\pi_0 \rightarrow^E v_{\pi_0} \rightarrow^I \pi_1 \rightarrow^E v_{\pi_1} \rightarrow^I ... \rightarrow^I \pi^* \rightarrow^E v_{*}
$$
...
- Value Function을 정확하게 구했다고 해서 Optimal Value Function을 구할 수 있는 것은 아니다. 단지 현재 Policy가 가지는 정확한 Value Function을 알아냈을 뿐이다.

Policy Evaluation은 첫 번째 문제, 즉 아래와 같은 Bellman Equation을 모든 State에 대해 적용하여 Value Function을 업데이트 하는 과정이다. 
Policy의 Value Function을 정확하게 구해 Policy가 얼마나 좋은지 평가하는 과정이라는 점에서 Policy Evaluation이라는 표현이 사용되었다고 할 수 있다.
...
위의 업데이트 식에 따르면 모든 $$s \in S$$에 대해 $$v_k = v_\pi$$는 Fixed Point가 되고, 따라서 $$k \rightarrow \infty$$라면 $$v_k$$가 $$v_\pi$$에 수렴하는 것이 보장된다고 한다. 즉, 모든 State에 대해 위의 식에 따라 반복적으로 Value를 업데이트하게 되면 임의의 $$v_0$$에서 출발했다 할지라도 $$v_\pi$$에 매우 가까워 질 수 있다는 것이다.
...
Policy Iteration은 수렴성이 보장되지만 (1) 모든 State에 대해 Value를 구해야 한다는 점 (2) Value Function이 수렴할 때까지 무한히 반복해야 한다는 점 때문에 연산량과 시간이 오래 걸린다는 단점을 가지고 있다. 이러한 단점을 해소하기 위해 다음과 같이 보다 효율적인 방법들이 제시되었다.
...
Value Iteration은 Policy가 달라지면 Value Function을 다시 구해야 한다는 점에서 정확한 Value를 구하는 데에 너무 많이 투자할 필요가 없다는 점에서 착안했다고 할 수 있다. Discounted Finite MDP 환경에서라면 Value Iteration를 통해 보다 빠른 수렴이 보장된다고 한다.
...
- Discount Factor는 $$0 \geqq \gamma < 1$$를 만족해야 한다.
...
DP를 사용하면 기존 방법에 비해 다음과 같은 장점을 가진다고 한다.
